...
-(void)sendJoinRoomRequest:(WebRTCSessionConfig*)sessionConfig
{
    ...
    [jsonPayload setObject:stackConfig.userName forKey:@"fromUID"];
    [jsonPayload setObject:sessionConfig.deviceType forKey:@"deviceType"];
    [jsonPayload setObject:@"bc" forKey:@"topic"];
    [jsonPayload setObject:@"rtcgsmdev" forKey:@"displayName"];
    ...
    [jsonHeaders setObject:stackConfig.trackingIdHeader forKey:@"x-tracking-id"];
    ...
    NSString *formattedUrl = [NSString stringWithFormat:@"%@/joinroom/roomids/%@?uid=%@", stackConfig.serverURL,sessionConfig.rtcgSessionId,  stackConfig.userName];
    ...
    [httpconn sendCreateJoinRoomRequest:jsonPayload _requestHeaders:jsonHeaders];
}
...
//    [jsonPayload setObject:sessionConfig.participantsInfo forKey:@"participantsInfo"];
    ...
    NSString *formattedUrl = [NSString stringWithFormat:@"%@/createroom?uid=%@", stackConfig.serverURL,  stackConfig.userName];
    NSLog(@"formattedUrl = %@",formattedUrl);
    httpconn = [[WebRTCHTTP alloc]initWithDefaultValue:formattedUrl _token:stackConfig.wsToken];
    httpconn.delegate = self;
    [httpconn sendCreateJoinRoomRequest:jsonPayload _requestHeaders:jsonHeaders];
...
- (void) createXMPPConnection:(NSString*)roomId
{
    NSArray *lines = [roomId componentsSeparatedByString: @"@"];
    _roomID = lines[0];
    ...
    [[XMPPWorker sharedInstance] setUserName:roomId];
...
//        [self.delegate showStatus:@"Reconnecting Network.."];
//        [sh disconnectForce];
//        [httpconn sendResourceRequest];
//        //[sh connectToSignallingServer:username credentials:encodedcredential resource:path];
//    }
}
...
- (void)onConnected
{
    ...
    if(isReconnecting)
    {
        ...
        [self.delegate showStatus:@"Network Connected"];
